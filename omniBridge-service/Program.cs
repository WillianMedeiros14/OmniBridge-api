...
using omniBridge_service.Data;
using omniBridge_service;

var builder = WebApplication.CreateBuilder(args);
var connectionString = builder.Configuration.GetConnectionString("OmniBridgeMicroservice");

builder.Services.AddDbContext<OmniBridgeContext>(opts =>
    opts.UseLazyLoadingProxies().UseNpgsql(connectionString));

builder.Services.AddIdentity<UserModel, IdentityRole>()
    .AddEntityFrameworkStores<OmniBridgeContext>()
...
{
    var db = scope.ServiceProvider.GetRequiredService<OmniBridgeContext>();
    db.Database.Migrate();
}
